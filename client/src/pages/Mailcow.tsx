...
export default function MailcowPage({ user }: { user: User }) {
  ...
  const { data: relayCredsData, refetch: refRelayCreds } = useQuery<{ host: string; port: number; username: string; password: string; note: string }>({
    queryKey: ["/api/smtp/relay/credentials"],
    queryFn:  () => fetch("/api/smtp/relay/credentials").then((r) => r.json()),
  });
  const [regenerating, setRegenerating] = useState(false);
  async function regenerateRelayCreds() {
    setRegenerating(true);
    try {
      await fetch("/api/smtp/relay/credentials/regenerate", { method: "POST" });
      refRelayCreds();
      toast("Relay secret regenerated — update Mailcow relay config with the new password");
    } catch { toast("Regenerate failed", false); }
        ...
        <div className="space-y-3">
          ...
          {relayCredsData && (
            <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-2.5 bg-[hsl(var(--muted)/0.4)] border-b border-[hsl(var(--border)/0.5)]">
                <div className="flex items-center gap-1.5">
                  <Link2 size={13} className="text-[hsl(var(--primary))]" />
                  <span className="text-xs font-semibold text-[hsl(var(--foreground))]">Relay Credentials</span>
                  <span className="text-[10px] text-[hsl(var(--muted-foreground))]">— paste into Mailcow → System → Configuration → Relayhost</span>
                </div>
                <button
                  onClick={regenerateRelayCreds}
                  disabled={regenerating}
                  data-testid="button-regenerate-relay-secret"
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-lg border border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] transition disabled:opacity-50"
                >
                  {regenerating
